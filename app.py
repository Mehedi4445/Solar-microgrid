...
Auth helpers are inlined here to avoid Streamlit Cloud module path issues.
...
if not os.path.exists(MODEL_PATH):
    with st.spinner("🤖 First-time setup: Training AI model on solar dataset… (~30 seconds)"):
        from train_model import train
        train()
    st.success("✅ Model trained and ready!")
    st.rerun()

# ── Inlined auth helpers (avoids utils/ import issues on Streamlit Cloud) ─────
